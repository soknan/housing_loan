...
class RptLoanInActiveController extends BaseController
{
    public function index()
    {
        //$data['reportHistory'] = $this->_reportHistory();
        return $this->renderLayout(
            \View::make('battambang/loan::rpt_loan_inactive.index')
        );
    }
    ...
    public function report()
    {
        ...
        $condition.=" and ln_client.id not in(select dc.ln_client_id from ln_disburse_client dc inner join ln_disburse d
        on d.id = dc.ln_disburse_id where d.disburse_date BETWEEN
                        STR_TO_DATE('".$data['date_from']." " . " 00:00:00" . "','%Y-%m-%d %H:%i:%s')
                        AND STR_TO_DATE('".$data['date_to']." " . " 23:59:59" . "','%Y-%m-%d %H:%i:%s') ) ";
...
// User action
       ...
       //exit;
        ...
        \Report::setReportName('Loan_Inactive')
            ->setDateFrom($data['date_from'])
        ->setDateTo($data['date_to']);
        return \Report::make('rpt_loan_inactive/source', $data,'loan_inactive');
