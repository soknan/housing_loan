...
class RptWriteOffController extends BaseController
{
    public function indexIn()
    {
        //$data['reportHistory'] = $this->_reportHistory();
        return $this->renderLayout(
            \View::make('battambang/loan::rpt_write_off.indexIn')
        );
    }
    ...
    public function indexEnd()
    {
        //$data['reportHistory'] = $this->_reportHistory();
        return $this->renderLayout(
            \View::make('battambang/loan::rpt_write_off.indexEnd')
        );
    }
    ...
    public function reportIn()
    {
        ...
        $data['from'] = \Carbon::createFromFormat('d-m-Y',Input::get('from'))->toDateString();
        $data['to'] = \Carbon::createFromFormat('d-m-Y',Input::get('to'))->toDateString();
        ...
        $condition = ' 1=1 ';
        $date= " AND p.activated_at BETWEEN
...
GROUP BY ln_disburse_client.id
        ...
        \Report::setReportName('Loan_Writeoff_End')
            ->setDateFrom($data['as_date']);
        return \Report::make('rpt_write_off/source_end', $data,'loan_Write-off-end');
