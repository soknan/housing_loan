...
class RptLoanBreakDownCurrencyController extends BaseController
{
    public function index()
    {
        //$data['reportHistory'] = $this->_reportHistory();
        return $this->renderLayout(
            \View::make('battambang/loan::rpt_loan_breakdown_currency.index')
        );
    }
...
COUNT(ln_disburse_client_id) as num_account,
...
// User action
        \Event::fire('user_action.report', array('rpt_breakdown_currency'));
        if (count($sql) <= 0) {
       ...
       //exit;
        ...
        \Report::setReportName('Loan_Breakdown_Currency')
            ->setDateFrom($data['as_date']);
        return \Report::make('rpt_loan_breakdown_currency/source', $data,'loan_breakdown_currency');
    }
