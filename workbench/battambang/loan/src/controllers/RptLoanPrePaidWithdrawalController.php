...
class RptLoanPrePaidWithdrawalController extends BaseController
{
    public function index()
    {
        //$data['reportHistory'] = $this->_reportHistory();
        return $this->renderLayout(
            \View::make('battambang/loan::rpt_loan_prepaid_withdrawal.index')
        );
    }
...
INNER JOIN ln_perform on ln_perform.ln_disburse_client_id =ln_pre_paid.ln_disburse_client_id and ln_perform.created_at = ln_pre_paid.created_at
and ln_perform.perform_type='repayment' and ln_perform.repayment_type <>'fee'
        where $condition
        order by ln_pre_paid.ln_disburse_client_id
        ");
// User action
        \Event::fire('user_action.report', array('rpt_loan_prepaid_withdrawal'));
       ...
       //exit;
        ...
        \Report::setReportName('Loan_Pre-Paid Withdrawal')
            ->setDateFrom($data['date_from'])
        ->setDateTo($data['date_to']);
        return \Report::make('rpt_loan_prepaid_withdrawal/source', $data,'loan_prepaid_withdrawal');
