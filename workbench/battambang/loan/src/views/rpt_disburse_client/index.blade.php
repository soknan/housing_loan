...
@section('content')
{{Former::open( route('loan.rpt_disburse_client.report'))->method('POST')}}
<?php echo FormPanel2::make(
    'General',
    Former::select('cp_office_id[]','Branch Office',GetLists::getSubBranchListNoAjax(UserSession::read()->package))
        ->multiple('multiple')
        ->required()
        .''
   ...
   .Former::select('exchange_rate','Exchange Rate')
        ...
        ->class('select2')
        ->required().''
    .Former::text('from','Date From',date('d-m-Y'))->append('dd-mm-yyyy')->required().''
    .Former::text('to','Date To',date('d-m-Y'))->append('dd-mm-yyyy')->required()
)
?>
...
@section('js')
<?php
echo DatePicker::make('from');
echo DatePicker::make('to');
?>
@stop
