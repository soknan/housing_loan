...
@section('content')
{{Former::open( route('loan.rpt_loan_late.report'))->method('POST')}}
<?php echo FormPanel2::make(
    ...
    .Former::select('ln_lv_repay_frequency','Repay Frequency')
        ...
        .''
    ,
    Former::select('operator','Operator')
        ->options(array('all'=>'All'))
        ->options(LookupValueList::getOperator())
        ->class('select2')
    .''
    .Former::text('late','Late').''
    ...
    .Former::select('exchange_rate','Exchange Rate')
        ...
        ->class('select2')
        ->required().''
    .Former::text('as_date','Date',date('d-m-Y'))->append('dd-mm-yyyy')->required().''
