...
echo FormPanel2::make(
    ...
    ,
    Former::select('ln_lv_marital_status', 'Status', \LookupValueList::getBy('marital status'), $row->ln_lv_marital_status)->required() . ''
    . Former::select('ln_lv_education', 'Education', \LookupValueList::getBy('education'), $row->ln_lv_education)->required()
    . Former::textarea('education_des', 'Education Des', $row->education_des)
    . Former::select('ln_lv_title', 'Position', \LookupValueList::getBy('title'), $row->ln_lv_title)->required()
...
echo FormPanel2::make(
    'ID Type',
    Former::select('ln_lv_id_type', 'ID Type', \LookupValueList::getBy('id type'), $row->ln_lv_id_type)->required() . ''
